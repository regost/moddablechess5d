...
void LoadPgn::ParsePNGfile(Node<Turn>* prime,const string& path, uint16_t primeTimeline, uint16_t numberOfnegativeT)
{
	...
	while (std::getline(file, line)) {
		...
		for (size_t j = 0; j < line.size(); ++j) {
			if (line[j] == '/' && blacksmove == 0) {
				...
				ActionAndString<Turn> turn(line.substr(firstPart, (j - firstPart)), primeTimeline, numberOfnegativeT, blacksmove);
				firstPart = none;
